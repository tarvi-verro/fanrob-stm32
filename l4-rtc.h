...
struct rtc_cr {
	uint32_t wucksel : 3, tsedge : 1, refckon : 1, bypshad : 1, fmt : 1,
		 ...
		 bkp : 1, cosel : 1, pol : 1, osel : 2, coe : 1, itse : 1, : 7;
};
...
struct rtc_tafcr { // TODO: dafuq is this? ≡ rtc_tampcr?
	uint32_t tamp1e : 1, tamp1trg : 1, tampie : 1, tamp2e : 1,
...
struct rtc_reg {
	...
	uint32_t or;			// 0x4C
	uint32_t bkp[0x20];		// 0x50 to 0xCC
};

_Static_assert (offsetof(struct rtc_reg, bkp) == 0x50 && sizeof(struct rtc_reg) == 0xD0,
